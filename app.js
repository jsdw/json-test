...
Elm.JsonTest.make = function (_elm) {
   ...
   var row = F3(function (name,id,input) {
      ...
      _U.list([A2($Html.td,
              ...
              ,A2($Html.td,_U.list([$Html$Attributes.$class("row-input")]),_U.list([input]))]));
   });
   var viewResponse = function (res) {
      ...
      var response = function (details) {
         ...
         _U.list([A3(row,"Status","",$Html.text(A2($Basics._op["++"],$Basics.toString(details.status),A2($Basics._op["++"],": ",details.statusText))))
                 ,A3(row,"Headers","",A2($Html.table,_U.list([]),A2($List.map,viewHeader,$Dict.toList(details.headers))))
   ...
   var isJust = function (m) {    var _p6 = m;if (_p6.ctor === "Nothing") {    return false;} else {    return true;}};
   ...
   var makeRequestGiven = function (model) {
      ...
      A2($Task.andThen,A2($Http.send,$Http.defaultSettings,request),function (_p7) {    return $Task.succeed(ResSuccess(_p7));}),
      function (_p8) {
         return $Task.succeed(ResError(_p8));
      });
   ...
   var update = F2(function (action,model) {
      ...
      var _p9 = action;
      switch (_p9.ctor)
      {case "Path": return noFx(_U.update(model,{path: _p9._0}));
         case "ReqVerb": return noFx(_U.update(model,{verb: _p9._0}));
         case "ReqBody": return noFx(_U.update(model,{req: _p9._0}));
         case "ResSuccess": return noFx(_U.update(model,{loading: false,res: $Maybe.Just($Result.Ok(_p9._0))}));
         case "ResError": return noFx(_U.update(model,{loading: false,res: $Maybe.Just($Result.Err(_p9._0))}));
         default: return {ctor: "_Tuple2",_0: _U.update(model,{loading: true,res: $Maybe.Nothing}),_1: makeRequestGiven(model)};}
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("JSON Tester")]))
              ,A2($Html.table,
