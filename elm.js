...
Elm.JsonTest = Elm.JsonTest || {};
Elm.JsonTest.make = function (_elm) {
   "use strict";
   _elm.JsonTest = _elm.JsonTest || {};
   if (_elm.JsonTest.values) return _elm.JsonTest.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var row = F3(function (name,id,input) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("row")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("row-name")]),
              _U.list([A2($Html.label,_U.list([$Html$Attributes.$for(id)]),_U.list([$Html.text(name)]))]))
              ,A2($Html.div,_U.list([$Html$Attributes.$class("row-input")]),_U.list([input]))]));
   });
   var viewResponse = function (res) {
      var viewBody = function (val) {
         var _p1 = val;
         if (_p1.ctor === "Text") {
               return $Html.text(_p1._0);
            } else {
               return $Html.text("Cannot view body: not a string");
            }
      };
      var viewHeader = function (_p2) {    var _p3 = _p2;return A3(row,_p3._0,"",$Html.text(_p3._1));};
      var response = function (details) {
         return A2($Html.div,
         _U.list([]),
         _U.list([A3(row,"Status","",$Html.text(A2($Basics._op["++"],$Basics.toString(details.status),A2($Basics._op["++"],": ",details.statusText))))
                 ,A3(row,"Headers","",A2($Html.div,_U.list([]),A2($List.map,viewHeader,$Dict.toList(details.headers))))
                 ,A3(row,"Body","",viewBody(details.value))]));
      };
      var responseError = $Html.text("There was a low level error processing your request.");
      var noResponse = $Html.text("No request made");
      var _p4 = res;
      if (_p4.ctor === "Nothing") {
            return noResponse;
         } else {
   ...
   var ResError = function (a) {    return {ctor: "ResError",_0: a};};
   var ResSuccess = function (a) {    return {ctor: "ResSuccess",_0: a};};
   var makeRequestGiven = function (model) {
      var headers = _U.list([{ctor: "_Tuple2",_0: "Content-Type",_1: "application/json"}]);
      var request = {verb: model.verb,headers: headers,url: model.path,body: $Http.string(model.req)};
      var action = A2($Task.onError,
      A2($Task.andThen,A2($Http.send,$Http.defaultSettings,request),function (_p6) {    return $Task.succeed(ResSuccess(_p6));}),
      function (_p7) {
         return $Task.succeed(ResError(_p7));
      });
      return $Effects.task(action);
   };
   var update = F2(function (action,model) {
      var noFx = function (m) {    return {ctor: "_Tuple2",_0: m,_1: $Effects.none};};
      var _p8 = action;
      switch (_p8.ctor)
      {case "Path": return noFx(_U.update(model,{path: _p8._0}));
         case "ReqVerb": return noFx(_U.update(model,{verb: _p8._0}));
         case "ReqBody": return noFx(_U.update(model,{req: _p8._0}));
         case "ResSuccess": return noFx(_U.update(model,{res: $Maybe.Just($Result.Ok(_p8._0))}));
         case "ResError": return noFx(_U.update(model,{res: $Maybe.Just($Result.Err(_p8._0))}));
         default: return {ctor: "_Tuple2",_0: model,_1: makeRequestGiven(model)};}
   });
   var ReqBody = function (a) {    return {ctor: "ReqBody",_0: a};};
   var ReqVerb = function (a) {    return {ctor: "ReqVerb",_0: a};};
   var Path = function (a) {    return {ctor: "Path",_0: a};};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("container")]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("inputs")]),
              _U.list([A3(row,
                      "Path",
                      "input-path",
                      A2($Html.input,_U.list([$Html$Attributes.id("input-path"),A2(onInput,address,Path),$Html$Attributes.value(model.path)]),_U.list([])))
                      ,A3(row,
                      "Method",
                      "input-method",
                      A2($Html.select,
                      _U.list([$Html$Attributes.id("input-method"),A2(onInput,address,ReqVerb)]),
                      _U.list([A2($Html.option,_U.list([$Html$Attributes.selected(_U.eq(model.verb,"GET"))]),_U.list([$Html.text("GET")]))
                              ,A2($Html.option,_U.list([$Html$Attributes.selected(_U.eq(model.verb,"POST"))]),_U.list([$Html.text("POST")]))])))
                      ,A3(row,"JSON","input-json",A2($Html.textarea,_U.list([A2(onInput,address,ReqBody),$Html$Attributes.value(model.req)]),_U.list([])))]))
              ,A2($Html.div,
              ...
              ,A2($Html.div,_U.list([$Html$Attributes.$class("response")]),_U.list([viewResponse(model.res)]))]));
   });
   var Model = F4(function (a,b,c,d) {    return {path: a,req: b,verb: c,res: d};});
   var init = {ctor: "_Tuple2",_0: A4(Model,"","","GET",$Maybe.Nothing),_1: $Effects.none};
   return _elm.JsonTest.values = {_op: _op,init: init,update: update,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Effects = Elm.Effects.make(_elm),
   $Html = Elm.Html.make(_elm),
   $JsonTest = Elm.JsonTest.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $JsonTest.init,update: $JsonTest.update,view: $JsonTest.view,inputs: _U.list([])});
   var main = app.html;
